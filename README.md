...
`flow.toml` 裡的 selector 全是 `TODO_` 佔位符，**必須你自己填**。momo 的購物車 / 結帳
頁沒有公開文件、DOM 也會改版，任何寫死的 selector 都只是猜測。只要還有 `TODO`，
下單流程就會被閘門直接擋下。

抓法：登入 momo 走一次購買流程，在每個要點的按鈕上右鍵 →「檢查」→ Copy selector。
優先用 `id`，其次是穩定的 `data-*` 屬性；class 名稱最容易在改版時失效。
